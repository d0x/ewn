...
(deftest moegliche-zug-ziele-test
  (testing "moegliche zuege für einen stein"
    (is (= (moegliche-zug-ziele "↘️" {:x 4 :y 4}) '()))
    (is (= (moegliche-zug-ziele "↘️" {:x 4 :y 0}) '({:x 4 :y 1})))
    (is (= (moegliche-zug-ziele "↘️" {:x 0 :y 4}) '({:x 1 :y 4})))
    (is (= (moegliche-zug-ziele "↘️" {:x 0 :y 0}) '({:x 1 :y 0} {:x 0 :y 1} {:x 1 :y 1})))

    (is (= (moegliche-zug-ziele "↖️" {:x 0 :y 0}) '()))
    (is (= (moegliche-zug-ziele "↖️" {:x 0 :y 4}) '({:x 0 :y 3})))
    (is (= (moegliche-zug-ziele "↖️" {:x 4 :y 0}) '({:x 3 :y 0})))
    (is (= (moegliche-zug-ziele "↖️" {:x 4 :y 4}) '({:x 3 :y 4} {:x 4 :y 3} {:x 3 :y 3})))))

(deftest moegliche-zuege-test
  (testing "moegliche zuege für einen stein"
    (is (= (moegliche-zuege "↘️" [{:x 3, :y 3} {:x 2, :y 2}]) '({:from {:x 3, :y 3}, :to {:x 4, :y 3}}
...
(deftest zug-is-shortes-path-test
  (testing
    (is (= (zug-is-shortes-path "↖️" {:x 4 :y 4} {:x 3 :y 3}) true))
    (is (= (zug-is-shortes-path "↖️" {:x 4 :y 0} {:x 3 :y 0}) true))
    (is (= (zug-is-shortes-path "↖️" {:x 4 :y 4} {:x 3 :y 4}) false))
    (is (= (zug-is-shortes-path "↖️" {:x 4 :y 4} {:x 4 :y 3}) false))
    (is (= (zug-is-shortes-path "↖️" {:x 3 :y 4} {:x 2 :y 3}) true))
    (is (= (zug-is-shortes-path "↖️" {:x 3 :y 4} {:x 3 :y 3}) true))
    (is (= (zug-is-shortes-path "↖️" {:x 2 :y 2} {:x 2 :y 1}) false))
    (is (= (zug-is-shortes-path "↖️" {:x 4 :y 2} {:x 4 :y 1}) false))
    (is (= (zug-is-shortes-path "↖️" {:x 0 :y 2} {:x 0 :y 1}) true))
    (is (= (zug-is-shortes-path "↖️" {:x 2 :y 0} {:x 1 :y 0}) true))
    (is (= (zug-is-shortes-path "↘️" {:x 2 :y 2} {:x 3 :y 3}) true))

    (is (= (zug-is-shortes-path "↘️" {:x 1 :y 2} {:x 2 :y 2}) true))
    (is (= (zug-is-shortes-path "↘️" {:x 1 :y 2} {:x 1 :y 3}) false))
    (is (= (zug-is-shortes-path "↘️" {:x 1 :y 4} {:x 2 :y 4}) true))

    (is (= (zug-is-shortes-path "↘️" {:x 2 :y 4} {:x 3 :y 4}) true))))
...
(deftest stein-direct-enemy-count-test
  (testing
    (is (= (stein-direct-enemy-count board "↖️" :opponent {:x 1 :y 1}) 3))
    (is (= (stein-direct-enemy-count board "↖️" :opponent {:x 0 :y 1}) 1))
    (is (= (stein-direct-enemy-count board "↖️" :opponent {:x 1 :y 0}) 1))
    (is (= (stein-direct-enemy-count board "↖️" :opponent {:x 4 :y 4}) 0))
    ))
